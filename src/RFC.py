...
def define_model_pipeline(X):
    ...
    Parameters:
    X: The dataset used to determine the preprocessing steps.

    Returns:
    A sklearn Pipeline object configured for preprocessing and modeling.
    """
...
def fit_and_evaluate(model, X_train, X_test, y_train, y_test):
    """
    Fit the model to the training data and evaluate its performance on the test set.

    Parameters:
    model (Pipeline): The model pipeline to be fitted and evaluated.
